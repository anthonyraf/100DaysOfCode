...
fn main() {
    let mut attempts = 3;
    let win = randrange(0,10); // The winner number
    ...
    'mainloop: loop {
        ...
        if input > win {
            ...
            println!("Congratulations !! {} is the right number", win);
            break 'mainloop;
        }
...
fn ask_number(attempts: i32) -> () {
    print!("Guess the number between 0 and 10 ({} attempts remain) : ", attempts);
